...
export function createTokensRouter(
   ...
   */
  router.get("/:symbol/batch", async (req, res) => {
    try {
  ...
  router.get("/:symbol", async (req, res) => {
    try {
      const token = await prices.getTokenBySymbol(req.params.symbol);
      if (!token) {
        res.status(404).json({ error: "Token not found" });
        return;
      }
  ...
  router.get("/:symbol/impact", async (req, res) => {
    try {
      const symbol = req.params.symbol.toLowerCase();
      const token = await prices.getTokenBySymbol(symbol);
      if (!token) {
  ...
  router.get("/:symbol/faq", async (req, res) => {
    try {
      const symbol = req.params.symbol.toLowerCase();
      const token = await prices.getTokenBySymbol(symbol);
      if (!token) {
  ...
  router.get("/:symbol/sentiment", async (req, res) => {
    try {
      ...
      const socialRepo = new SocialRepository();
      const latest = await socialRepo.getLatest(token.symbol.toUpperCase());
      const history = await socialRepo.getHistory(token.symbol.toUpperCase(), 24);
      const change = await socialRepo.getMentionChange(token.symbol.toUpperCase());
  ...
  router.get("/:symbol/related", async (req, res) => {
    try {
      const symbol = req.params.symbol.toLowerCase();
      const token = await prices.getTokenBySymbol(symbol);
      if (!token) {
