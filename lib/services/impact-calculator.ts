import type { ImpactRepository } from "@/lib/db/impact-repository";
import type { NewsRepository, ArticleRow } from "@/lib/db/news-repository";
import type { PriceRepository } from "@/lib/db/price-repository";
import type { NewsClassifier, ClassificationResult } from "./news-classifier";
...
export class ImpactCalculator {
  ...
  async getArticleImpact(articleId: number): Promise<ArticleImpact | null> {
    ...
    const categoryRow = await this.impactRepo.getCategoryByArticleId(articleId);
  ...
  async computeImpactEvents(limit: number = 100): Promise<number> {
    ...
    const inserts: import("@/lib/db/impact-repository").ImpactEventInsert[] = [];
