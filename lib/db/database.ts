...
/**
 * Get a connection pool for use in long-running contexts (dev server, cron jobs).
 * For simple one-off queries in serverless functions, prefer `getSQL()` instead.
 */
...
/**
 * Get a Neon SQL tagged-template function for simple serverless queries.
 * More efficient for single queries in serverless/edge contexts (no pool overhead).
 */
